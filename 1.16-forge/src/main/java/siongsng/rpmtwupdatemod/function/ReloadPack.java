...
public class ReloadPack {
    Path CACHE_DIR = Paths.get(System.getProperty("user.home") + "/.rpmtw/1.16");
    Path PACK_NAME = CACHE_DIR.resolve("RPMTW-1.16.zip");
    String Update_Path = CACHE_DIR + "/Update.txt";
    ...
    public ReloadPack() {
        try {
            ...
            fr.close();
            if (Integer.parseInt(Latest_ver_n) > Old_ver + Configer.Update_interval.get()) {
